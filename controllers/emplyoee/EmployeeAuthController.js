...
module.exports = {
    login: async function(req, resp, next) {
        ...
        if (!matched) {
            ...
            User.findOne({ email: req.body.email, role: "employee" }, '', function(err, employee) {
                if (err) {
                    ...
                    if (chkval) {
                        ...
                        const tokendata = { user_id: employee._id, user_email: employee.email, role: employee.role }
                        const token = jwt.sign(tokendata, CONFIG.CONSTANTS.JWT_SECRET, options);

                        resp.status(200).json({ status: 'success', employee: employee, token: token, message: 'Logged in successfully.' });
                    } else {
